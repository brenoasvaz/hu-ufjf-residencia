...
async function seed() {
  ...
  try {
    ...
    // ========== ATIVIDADES SEMANAIS - R1 CENTRO CIRÚRGICO 1 ==========
    console.log("\ud83d\udcda Inserindo atividades semanais - R1 CC1...");
    ...
    const [cc1_2] = await db.insert(weeklyActivities).values({ diaSemana: 1, horaInicio: "08:00", horaFim: "10:00", titulo: "CC HU SC Ombro", descricao: "Centro Cirúrgico Santa Casa - Ombro (Mota)", local: "HU Santa Casa", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_2.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_3] = await db.insert(weeklyActivities).values({ diaSemana: 1, horaInicio: "15:00", horaFim: "17:00", titulo: "CC HU SC Coluna", descricao: "Centro Cirúrgico Santa Casa - Coluna (Vitor)", local: "HU Santa Casa", recorrente: 1 });
    ...
    // Quinta-feira CC1
    const [cc1_10] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "10:00", horaFim: "12:00", titulo: "Ambulatório Coluna", local: "Ambulatório", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_10.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_11] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "13:00", horaFim: "16:00", titulo: "CC HU DB Coluna", descricao: "Centro Cirúrgico Dom Bosco - Coluna (Marcus)", local: "HU Dom Bosco", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_11.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_12] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "18:00", horaFim: "19:00", titulo: "Estudo Dirigido", local: "Sala de Estudos", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_12.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_13] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "19:00", horaFim: "23:00", titulo: "R1 - HPS", descricao: "Plantão HPS", local: "HPS", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_13.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);

    // Sexta-feira CC1
    const [cc1_14] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "08:00", horaFim: "10:00", titulo: "Visita HU SC", local: "HU Santa Casa", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_14.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_15] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "09:00", horaFim: "12:00", titulo: "CC HU SC Tumor", descricao: "Centro Cirúrgico Santa Casa - Tumor (Sávio)", local: "HU Santa Casa", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_15.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_16] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "15:00", horaFim: "17:00", titulo: "CC HU SC Trauma", descricao: "Centro Cirúrgico Santa Casa - Trauma (Daniel)", local: "HU Santa Casa", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_16.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);
    
    const [cc1_17] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "19:00", horaFim: "23:00", titulo: "R1 - HPS", descricao: "Plantão HPS", local: "HPS", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_17.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);

    // Sábado CC1
    const [cc1_18] = await db.insert(weeklyActivities).values({ diaSemana: 6, horaInicio: "19:00", horaFim: "23:00", titulo: "R1 - HPS", descricao: "Plantão HPS", local: "HPS", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc1_18.insertId, anoResidencia: "R1", bloco: "CC1", opcional: 0 }]);

    // ========== ATIVIDADES SEMANAIS - R1 CENTRO CIRÚRGICO 2 ==========
    console.log("\ud83d\udcda Inserindo atividades semanais - R1 CC2...");
    ...
    const [cc2_2] = await db.insert(weeklyActivities).values({ diaSemana: 1, horaInicio: "10:00", horaFim: "12:00", titulo: "Ambulatório Coluna", descricao: "Ambulatório de Coluna (Vitor)", local: "Ambulatório", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_2.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_3] = await db.insert(weeklyActivities).values({ diaSemana: 1, horaInicio: "13:00", horaFim: "15:00", titulo: "Ambulatório Ombro", descricao: "Ambulatório de Ombro (Mota)", local: "Ambulatório", recorrente: 1 });
    ...
    const [cc2_9] = await db.insert(weeklyActivities).values({ diaSemana: 3, horaInicio: "12:00", horaFim: "14:00", titulo: "Ambulatório Ombro", descricao: "Ambulatório de Ombro (Adriano)", local: "Ambulatório", recorrente: 1 });
    ...
    // Quinta-feira CC2
    const [cc2_11] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "10:00", horaFim: "12:00", titulo: "Ambulatório Mão", descricao: "Ambulatório de Mão (Arnaldo)", local: "Ambulatório", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_11.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_12] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "13:00", horaFim: "16:00", titulo: "CC HU DB Mão", descricao: "Centro Cirúrgico Dom Bosco - Mão (Arnaldo)", local: "HU Dom Bosco", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_12.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_13] = await db.insert(weeklyActivities).values({ diaSemana: 4, horaInicio: "19:00", horaFim: "23:00", titulo: "R1 - HPS", descricao: "Plantão HPS", local: "HPS", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_13.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);

    // Sexta-feira CC2
    const [cc2_14] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "08:00", horaFim: "10:00", titulo: "Visita HU SC", local: "HU Santa Casa", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_14.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_15] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "10:00", horaFim: "12:00", titulo: "Ambulatório Ombro", descricao: "Ambulatório de Ombro (Adriano)", local: "Ambulatório", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_15.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_16] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "13:00", horaFim: "15:00", titulo: "CC HU DB Mão", descricao: "Centro Cirúrgico Dom Bosco - Mão (Breno)", local: "HU Dom Bosco", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_16.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_17] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "16:00", horaFim: "18:00", titulo: "Ambulatório Mão", descricao: "Ambulatório de Mão (Breno)", local: "Ambulatório", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_17.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_18] = await db.insert(weeklyActivities).values({ diaSemana: 5, horaInicio: "19:00", horaFim: "23:00", titulo: "R1 - HPS", descricao: "Plantão HPS", local: "HPS", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_18.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);

    // Sábado CC2
    const [cc2_19] = await db.insert(weeklyActivities).values({ diaSemana: 6, horaInicio: "07:00", horaFim: "10:00", titulo: "CC HU SC Quadril", descricao: "Centro Cirúrgico Santa Casa - Quadril (Daniel)", local: "HU Santa Casa", recorrente: 1 });
    await db.insert(activityAudiences).values([{ activityId: cc2_19.insertId, anoResidencia: "R1", bloco: "CC2", opcional: 0 }]);
    
    const [cc2_20] = await db.insert(weeklyActivities).values({ diaSemana: 6, horaInicio: "19:00", horaFim: "23:00", titulo: "R1 - HPS", descricao: "Plantão HPS", local: "HPS", recorrente: 1 });
    ...
    // ========== RODÍZIOS ==========    console.log("📅 Inserindo rodízios de janeiro/2026...");
